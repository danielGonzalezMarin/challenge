...
function crearClaseViajero() {
    class Viajero {
        ...
        getCompañeros() {
            ...
            // Tu código aca:
            //let tamaño = this.compañeros.length;
            let nombres = [];
            this.compañeros = JSON.stringify(this.compañeros);
            //Convirtiendo a objeto javascript
            let data = JSON.parse(this.compañeros);

            for (let x in data) {
                nombres.push(data[x].nombre);
            }
